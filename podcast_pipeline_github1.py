...
class PodcastProcessor:
    def __init__(self):
        ...
        self.emailed = self.load_emailed()
        self.new_episodes = 0
    ...
    def load_processed(self):
        """Load processed episodes with cleanup"""
        try:
            ...
            if isinstance(data, list):
                ...
                # Clean old entries (30+ days) and return as set
                cutoff = datetime.now() - timedelta(days=30)
                valid_hashes = set()
                for episode_hash, info in data.items():
                    try:
                        processed_date = datetime.fromisoformat(info.get('processed', '1900-01-01'))
                        if processed_date > cutoff:
                            valid_hashes.add(episode_hash)
                    except:
                        continue
                return valid_hashes
            else:
    ...
    def load_emailed(self):
        """Load emailed episodes to prevent duplicates"""
        try:
            if not os.path.exists(EMAILED_FILE):
                return set()
            
            with open(EMAILED_FILE, 'r') as f:
                data = json.load(f)
            
            # Handle both old dict format and convert to set for faster lookups
            if isinstance(data, dict):
                # Clean old entries (30+ days) to prevent file bloat
                cutoff = datetime.now() - timedelta(days=30)
                valid_hashes = set()
                
                for episode_hash, email_info in data.items():
                    try:
                        email_date = datetime.fromisoformat(email_info.get('emailed', '1900-01-01'))
            ...
            logger.error(f"Error loading emailed file: {e}")
            return set()

    def save_processed(self):
        """Save processed episodes"""
        try:
            # Convert set to dict with timestamp info
            processed_data = {}
            for episode_hash in self.processed:
                processed_data[episode_hash] = {
            ...
            logger.info(f"Saved {len(self.processed)} processed episodes")
        except Exception as e:
            logger.error(f"Error saving processed file: {e}")

    def save_emailed(self):
        """Save emailed episodes to prevent duplicates"""
        try:
            # Convert set to dict with timestamp info for file storage
            emailed_data = {}
            for episode_hash in self.emailed:
                emailed_data[episode_hash] = {
                    'emailed': datetime.now().isoformat(),
                    'version': '3.0'
                }
            
            with open(EMAILED_FILE, 'w') as f:
                json.dump(emailed_data, f, indent=2)
            logger.info(f"Saved {len(self.emailed)} emailed episodes")
        except Exception as e:
            logger.error(f"Error saving emailed file: {e}")

    def mark_as_emailed(self, episode_hash, title, feed_name):
        """Mark episode as emailed"""
        self.emailed.add(episode_hash)
        logger.info(f"Marked as emailed: {title} (hash: {episode_hash})")
        self.save_emailed()

    def is_already_emailed(self, episode_hash):
        """Check if episode was already emailed"""
        is_emailed = episode_hash in self.emailed
        if is_emailed:
            logger.info(f"Episode already emailed (hash: {episode_hash})")
        return is_emailed

    def is_already_processed(self, episode_hash):
        """Check if episode was already processed"""
        is_processed = episode_hash in self.processed
        if is_processed:
            logger.info(f"Episode already processed (hash: {episode_hash})")
        return is_processed

    def get_episode_hash(self, url, title="", published=""):
        """Generate unique hash for episode - include more identifying info"""
        # Use URL, title, and published date for more unique identification
        unique_string = f"{url}_{title}_{published}".encode('utf-8')
        return hashlib.sha256(unique_string).hexdigest()[:16]  # Longer hash for better uniqueness

    def is_recent_episode(self, published_date, max_days=10):
        """Check if episode is recent (changed to 10 days max)"""
        try:
...
"""
    ...
    def send_email(self, title, summary, date, episode_info):
        """Send formatted email summary (without transcript)"""
        try:
            ...
            if not all([email_from, email_to, email_password]):
                ...
                logger.error(f"EMAIL_FROM: {'✓' if email_from else '✗'}")
            ...
            # Debug: Check if email content creation works
            try:
                html_body, text_body = self.create_email_content(title, summary, date, episode_info)
                logger.info(f"Email content created successfully - HTML: {len(html_body)} chars, Text: {len(text_body)} chars")
            except Exception as content_error:
                logger.error(f"Error creating email content: {repr(content_error)}")
                return False
    ...
    def process_episode(self, entry, audio_url, feed_name, provider):
        """Process a single episode with enhanced duplicate prevention"""
        title = entry.title
        published = entry.get('published', 'Unknown')
        
        # Generate episode hash with published date for better uniqueness
        episode_hash = self.get_episode_hash(audio_url, title, published)
        
        logger.info(f"Processing: {title}")
        logger.info(f"Episode hash: {episode_hash}")
        
        # CRITICAL: Triple check for duplicates before any processing
        if self.is_already_processed(episode_hash):
            logger.info(f"SKIPPING - Already processed: {title}")
            return False
            
        if self.is_already_emailed(episode_hash):
            logger.info(f"SKIPPING - Already emailed: {title}")
            return False
        
        # Mark as processed IMMEDIATELY to prevent race conditions
        self.processed.add(episode_hash)
        self.save_processed()
        logger.info(f"Marked as processed: {episode_hash}")
        
        audio_file = os.path.join(DOWNLOAD_DIR, f"{episode_hash}.mp3")
        ...
        try:
            ...
            # FINAL CHECK: Ensure not already emailed (double safety)
            if self.is_already_emailed(episode_hash):
                logger.warning(f"DUPLICATE DETECTED at email stage: {title}")
                return False
            
            # Send email (without transcript)
            if self.send_email(title, summary, published, episode_info):
                # Mark as emailed IMMEDIATELY after successful email
                self.mark_as_emailed(episode_hash, title, feed_name)
    ...
    def process_feed(self, feed_name, feed_url):
        """Process a single RSS feed with enhanced duplicate prevention"""
        try:
            ...
            # Process up to 2 recent episodes
            for entry in feed.entries[:2]:
                title = entry.title
                published = entry.get('published', 'Unknown')
                
                # Skip if not recent (now 10 days max)
                if not self.is_recent_episode(published, max_days=10):
                    logger.info(f"Skipping old episode (>10 days): {title}")
                    continue
                ...
                # ENHANCED DUPLICATE CHECK: Check with published date included
                episode_hash = self.get_episode_hash(audio_url, title, published)
                
                # Multiple duplicate checks with detailed logging
                if episode_hash in self.processed:
                    logger.info(f"DUPLICATE PREVENTION: Already processed (hash: {episode_hash}): {title}")
                    continue
                
                if episode_hash in self.emailed:
                    logger.info(f"DUPLICATE PREVENTION: Already emailed (hash: {episode_hash}): {title}")
                    continue
                
                logger.info(f"NEW EPISODE DETECTED: {title} (hash: {episode_hash})")
    ...
    def run(self):
        """Main execution method with comprehensive duplicate prevention"""
        logger.info("Starting automated financial podcast pipeline...")
        logger.info(f"Processing episodes from last 10 days only")
        logger.info(f"Duplicate prevention: {len(self.processed)} episodes processed, {len(self.emailed)} episodes emailed")
        ...
        for feed_name, feed_url in RSS_FEEDS.items():
            ...
            time.sleep(5)  # Rate limiting between feeds
        ...
        logger.info(f"Financial pipeline complete: {self.new_episodes} new episodes in {duration}")
        logger.info(f"Total tracked: {len(self.processed)} processed, {len(self.emailed)} emailed")
