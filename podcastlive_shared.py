...
# Load environment variables directly (removed load_dotenv())
...
class PodcastProcessor:
    ...
    def load_processed(self):
        ...
        try:
            ...
            return [k for k, v in data.items() 
                   if datetime.fromisoformat(v.get('processed', '1900-01-01')) > cutoff]
            ...
            for episode_hash, email_info in data.items():
                try:
                    ...
                    if email_date > cutoff:
                        cleaned_data[episode_hash] = email_info
                except:
    ...
    def save_emailed(self):
        ...
        try:
            with open(EMAILED_FILE, 'w') as f:
                json.dump(self.emailed, f, indent=2)
    ...
    def is_already_emailed(self, episode_hash):
        ...
        return episode_hash in self.emailed

    def get_episode_hash(self, url, title=""):
        """Generate unique hash for episode"""
...
"""
    ...
    def process_episode(self, entry, audio_url, feed_name, provider):
        ...
        episode_hash = self.get_episode_hash(audio_url, title)
        ...
        try:
            ...
            if self.send_email(title, summary, published, episode_info):
                ...
                logger.info(f"Successfully processed and emailed: {title}")
                return True
            else:
    ...
    def process_feed(self, feed_name, feed_url):
        ...
        try:
            ...
            for entry in feed.entries[:2]:
                ...
                # Check if already emailed (duplicate prevention)
                if self.is_already_emailed(episode_hash):
                    logger.info(f"Already emailed: {title}")
                    continue
                
                # Process the episode
                if self.process_episode(entry, audio_url, feed_name, provider):
                    self.processed.append(episode_hash)
    ...
    def run(self):
        ...
        logger.info(f"Email duplicate prevention: {len(self.emailed)} episodes already emailed")
