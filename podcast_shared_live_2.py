...
class EmailedEpisodesDB:
    ...
    def claim_episode_atomic(self, url, title, published_date, feed_name):
        """
        ATOMIC OPERATION: Check if episode exists, if not, immediately claim it.
        Returns True if successfully claimed (not a duplicate), False if duplicate.
        This is the ONLY method that prevents duplicates.
        """
        with self._lock:
            ...
            try:
                with self._file_lock('r+') as f:
                    ...
                    if episode_key in episodes:
                        logger.info(f"DUPLICATE BLOCKED: {title}")
                        return False
                ...
                logger.info(f"EPISODE CLAIMED: {title}")
                return True
    ...
    def mark_as_emailed(self, url, title, published_date, feed_name, additional_info=None):
        """Mark episode as successfully emailed (for completed episodes only)"""
        with self._lock:
            episode_key = self._create_episode_key(url, title, published_date, feed_name)
...
"""
    ...
    def process_episode(self, entry, audio_url, feed_name, provider):
        """Process a single episode with ATOMIC duplicate prevention"""
        title = entry.title
        published = entry.get('published', 'Unknown')
        
        logger.info(f"Processing: {title}")
        
        # ATOMIC CLAIM: Only duplicate check needed - claims episode immediately
        if not self.email_db.claim_episode_atomic(audio_url, title, published, feed_name):
            return False  # Duplicate detected and blocked
        ...
        try:
            ...
            if self.send_email(title, summary, published, episode_info):
                ...
                self.email_db.mark_as_emailed(audio_url, title, published, feed_name, {
                    'provider': episode_info.get('provider', ''),
                    'email_subject': f"{feed_name}: {title}"
                })
    ...
    def process_feed(self, feed_name, feed_url):
        ...
        try:
            ...
            for entry in feed.entries[:3]:
                ...
                # Process the episode (includes atomic duplicate prevention)
    ...
    def run(self):
        ...
        if os.path.exists(LOCK_FILE):
            try:
                ...
                if lock_age > 7200:  # 2 hours - stale lock
