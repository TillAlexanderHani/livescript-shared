...
FIXED VERSION: Eliminates all duplicate email issues with robust tracking.
"""
import feedparser
import requests
import os
...
class EmailedEpisodesDB:
    """Robust database for tracking emailed episodes with file locking"""
    
    def __init__(self, db_file=EMAILED_DB_FILE):
    ...
    def _ensure_db_exists(self):
        ...
        if not os.path.exists(self.db_file):
            ...
            logger.info(f"Created new emailed episodes database: {self.db_file}")
    ...
    def _create_episode_key(self, url, title, published_date, feed_name):
        """Create a unique, deterministic key for an episode"""
        # Normalize inputs to handle slight variations
        normalized_url = url.strip().lower()
        normalized_title = re.sub(r'[^\w\s]', '', title.lower().strip())
        
        # Create composite key with multiple identifiers
        key_parts = [
            normalized_url,
            normalized_title,
            published_date.strip() if published_date else "",
            feed_name.strip()
        ]
        
        # Generate SHA-256 hash for consistent, unique identification
        key_string = "|".join(key_parts)
        return hashlib.sha256(key_string.encode('utf-8')).hexdigest()

    def is_emailed(self, url, title, published_date, feed_name):
        """Check if episode has been emailed (thread-safe)"""
        with self._lock:
            episode_key = self._create_episode_key(url, title, published_date, feed_name)
            ...
            try:
                with self._file_lock('r') as f:
                    ...
                    is_found = episode_key in episodes
                    if is_found:
                        episode_info = episodes[episode_key]
                        logger.info(f"DUPLICATE DETECTED: {title}")
                        logger.info(f"  Original email date: {episode_info.get('emailed_date')}")
                        logger.info(f"  Episode key: {episode_key}")
                    
                    return is_found
                    
            except Exception as e:
                logger.error(f"Error checking emailed status: {e}")
                return False

    def mark_as_emailed(self, url, title, published_date, feed_name, additional_info=None):
        """Mark episode as emailed with full tracking (thread-safe)"""
        with self._lock:
            episode_key = self._create_episode_key(url, title, published_date, feed_name)
            
            # Double-check before marking to prevent race conditions
            if self.is_emailed(url, title, published_date, feed_name):
                logger.warning(f"RACE CONDITION PREVENTED: Episode already marked as emailed: {title}")
                return False
            ...
            try:
                with self._file_lock('r+') as f:
                    ...
                    # Final check within file lock
                    if episode_key in episodes:
                        logger.warning(f"RACE CONDITION DETECTED IN FILE LOCK: {title}")
                        return False
                    ...
                    episodes[episode_key] = {
                        ...
                        "feed_name": feed_name,
                        "emailed_date": datetime.now().isoformat(),
                ...
                logger.info(f"MARKED AS EMAILED: {title}")
                logger.info(f"  Episode key: {episode_key}")
                logger.info(f"  Total tracked: {len(episodes) + 1}")
                
                return True
                
            except Exception as e:
                logger.error(f"Error marking episode as emailed: {e}")
                return False

    def cleanup_old_entries(self, days_to_keep=60):
            ...
            try:
                ...
                with self._file_lock('r+') as f:
                    ...
                    for key, info in episodes.items():
                        try:
                            emailed_date = datetime.fromisoformat(info.get('emailed_date', '1900-01-01'))
                            if emailed_date > cutoff_date:
                                cleaned_episodes[key] = info
                        except:
                ...
                removed_count = original_count - len(cleaned_episodes)
                if removed_count > 0:
                    logger.info(f"Database cleanup: removed {removed_count} old entries (>{days_to_keep} days)")
...
class PodcastProcessor:
    def __init__(self):
        ...
        self.email_db = EmailedEpisodesDB()
        self.new_episodes = 0
        
        # Cleanup old entries on startup
        self.email_db.cleanup_old_entries(days_to_keep=60)
    ...
    def is_recent_episode(self, published_date, max_days=7):
        """Check if episode is recent (reduced to 7 days to be more selective)"""
        try:
...
"""
    ...
    def send_email(self, title, summary, date, episode_info):
        """Send formatted email summary - WITH FINAL DUPLICATE CHECK"""
        try:
            # CRITICAL: Final duplicate check before sending email
            url = episode_info.get('url', '')
            feed_name = episode_info.get('feed', '')
            published = episode_info.get('published', '')
            
            if self.email_db.is_emailed(url, title, published, feed_name):
                logger.warning(f"FINAL DUPLICATE CHECK FAILED: Already emailed {title}")
                return False
            ...
            # IMMEDIATELY mark as emailed after successful send
            success = self.email_db.mark_as_emailed(url, title, published, feed_name, {
                'provider': episode_info.get('provider', ''),
                'email_subject': msg['Subject']
            })
            
            if not success:
                logger.error(f"WARNING: Failed to mark episode as emailed in database: {title}")
    ...
    def process_episode(self, entry, audio_url, feed_name, provider):
        """Process a single episode with ZERO duplicate tolerance"""
        title = entry.title
        published = entry.get('published', 'Unknown')
        
        logger.info(f"Processing: {title}")
        
        # CRITICAL: Multiple duplicate checks at every stage
        if self.email_db.is_emailed(audio_url, title, published, feed_name):
        ...
        try:
            ...
            # Another duplicate check after download (in case another process started)
            if self.email_db.is_emailed(audio_url, title, published, feed_name):
                logger.warning(f"RACE CONDITION DETECTED during processing: {title}")
                return False
            ...
            # FINAL duplicate check before email
            if self.email_db.is_emailed(audio_url, title, published, feed_name):
                logger.warning(f"FINAL DUPLICATE CHECK before email: Already processed {title}")
                return False
    ...
    def process_feed(self, feed_name, feed_url):
        """Process a single RSS feed with comprehensive duplicate prevention"""
        try:
            ...
            # Process up to 3 recent episodes (slightly more to catch new releases)
            for entry in feed.entries[:3]:
                title = entry.title
                published = entry.get('published', 'Unknown')
                
                # Skip if not recent (7 days max for more precision)
                if not self.is_recent_episode(published, max_days=7):
                    logger.info(f"Skipping old episode (>7 days): {title}")
                    continue
                ...
                if self.email_db.is_emailed(audio_url, title, published, feed_name):
                    logger.info(f"DUPLICATE PREVENTION: Already emailed: {title}")
                    continue
                ...
                if self.process_episode(entry, audio_url, feed_name, provider):
                    ...
                    time.sleep(3)  # Increased rate limiting for safety
                else:
    ...
    def run(self):
        """Main execution method with ZERO duplicate tolerance"""
        ...
        with self.pipeline_lock():
            logger.info("Starting DUPLICATE-FREE financial podcast pipeline...")
            logger.info(f"Processing episodes from last 7 days only")
            
            # Get database stats
            stats = self.email_db.get_stats()
            logger.info(f"Email database: {stats['total_episodes']} episodes tracked")
            ...
            for feed_name, feed_url in RSS_FEEDS.items():
                ...
                # Increased delay between feeds to prevent any race conditions
                time.sleep(8)
            
            duration = datetime.now() - start_time
            final_stats = self.email_db.get_stats()
            
            logger.info(f"PIPELINE COMPLETE: {self.new_episodes} new episodes processed in {duration}")
            logger.info(f"Database updated: {final_stats['total_episodes']} total episodes tracked")
            logger.info("ZERO DUPLICATE GUARANTEE: Every email sent is unique")
