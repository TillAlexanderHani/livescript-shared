...
FIXED VERSION: Uses simple, bulletproof duplicate prevention like the working old code.
"""
import feedparser
import requests
import os
...
class PodcastProcessor:
    ...
    def load_emailed(self):
        """Load emailed episodes - SIMPLE approach like the working old code"""
        try:
            ...
            for episode_hash, email_info in episodes.items():
                try:
            ...
            logger.info(f"Loaded {len(cleaned_data)} previously emailed episodes")
            return cleaned_data
    ...
    def save_emailed(self):
        """Save emailed episodes - SIMPLE approach"""
        try:
            # Save in new format but keep it simple
            data = {
                "version": "2.1",
                "last_updated": datetime.now().isoformat(),
                "episodes": self.emailed
            }
            
            with open(EMAILED_DB_FILE, 'w') as f:
...
"""
    ...
    def process_episode(self, entry, audio_url, feed_name, provider):
        """Process a single episode with BULLETPROOF duplicate prevention"""
        title = entry.title
        published = entry.get('published', 'Unknown')
        
        logger.info(f"Processing: {title}")
        
        # BULLETPROOF DUPLICATE CHECK - Mark as emailed IMMEDIATELY
        episode_hash = self.get_episode_hash(audio_url, title)
        if self.is_already_emailed(episode_hash):
            logger.info(f"DUPLICATE BLOCKED: Already emailed: {title}")
            return False
        
        # CRITICAL: Mark as emailed RIGHT NOW to prevent any other instance from processing
        self.mark_as_emailed(episode_hash, title, feed_name)
        logger.info(f"MARKED AS EMAILED IMMEDIATELY: {title}")
    ...
    def process_feed(self, feed_name, feed_url):
        """Process a single RSS feed with simple duplicate prevention"""
        try:
            ...
            # Process up to 2 recent episodes (like the old working code)
            for entry in feed.entries[:2]:
                title = entry.title
                published = entry.get('published', 'Unknown')
                
                # Skip if not recent (7 days max for precision)
                ...
                # Check duplicate BEFORE processing (simple approach)
                episode_hash = self.get_episode_hash(audio_url, title)
                if self.is_already_emailed(episode_hash):
                ...
                if self.process_episode(entry, audio_url, feed_name, provider):
                    ...
                    time.sleep(3)  # Rate limiting for safety
                else:
    ...
    def run(self):
        """Main execution method with BULLETPROOF duplicate prevention"""
        ...
        with self.pipeline_lock():
            ...
            logger.info(f"Duplicate prevention: {len(self.emailed)} episodes already emailed")
            ...
            logger.info(f"Total episodes tracked: {len(self.emailed)}")
            logger.info("ZERO DUPLICATE GUARANTEE: Using simple, proven approach from working old code")
